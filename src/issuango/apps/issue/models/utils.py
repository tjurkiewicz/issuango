        ...
        for attribute in self.get_all_attributes():
            ...
            if value is None:
                if attribute.required:
                    raise django.core.validators.ValidationError(_('{0} attribute cannot be blank').format(attribute.code))
            else:
                try:
                    attribute.validate_value(value)
                except django.core.validators.ValidationError as e:
                    raise django.core.validators.ValidationError(_('{0} attribute {1}').format(attribute.code, e))
